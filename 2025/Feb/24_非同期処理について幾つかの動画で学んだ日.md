...
## わかったこと（わからなかったこと）
- 非同期処理の必要性
  - JSはシングルスレッドで何も工夫しなければ同期処理(上から順に実行される。)
  - すると、例えばAPIで重いデータ取得してる時、画面が固まってしまう。
  - 非同期処理による解決
    - Ajaxコールバック関数→コールバック地獄(多重ネスト)
    - Promise, .thenで繋げることで多重ネストが解消→それでもまだ見にくい
    - async/await(同期処理likeにかけるようになった)
...
- 趣味(英語・読書・ジム)
  ...
  - 読書 0.5h<br>(estimated total read: 219books)
    - エンジニアの知的生産術
